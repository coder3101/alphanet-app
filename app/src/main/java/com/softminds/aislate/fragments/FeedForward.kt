...
class FeedForward : Fragment(), FetcherListener {


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_feed_forward, container, false)
    }

    override fun onActivityCreated(savedInstanceState: Bundle?) {
        super.onActivityCreated(savedInstanceState)
        //getActivity is now available
        MetaDataFetcher(this).fetch(ModelType.FEED_FORWARD)
    }
    ...
    override fun onFetchCompleted(metaDataList: List<ModelMetaData>) {
        ...
        if(feed_forward_recycler!=null) {
            feed_forward_recycler.setHasFixedSize(true)
            feed_forward_recycler.layoutManager = LinearLayoutManager(context)
            feed_forward_recycler.adapter = ExploreAdapter(metaDataList, context!!)
        }
    }
